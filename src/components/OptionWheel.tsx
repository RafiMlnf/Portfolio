...
const OptionWheel = ({
  ...
  useEffect(() => {
    ...
    const onWheel = (e: WheelEvent) => {
      ...
      const cfg = cfgRef.current;
      const rawDelta = e.deltaMode === 1 ? e.deltaY * 18 : e.deltaY;
      // Batasi tepat maksimum 1 lagu per sentakan scroll mouse (notch)
      const step = Math.sign(rawDelta) * Math.min(Math.abs(rawDelta) / cfg.rowH, 1);
